...
1. `int* bitmaskToArray(short int word[])` - this function takes the array of integers and returns a a bit vector array
2. `char* decodeString(int arr[])` - this function takes the bit vector array and returns the 8 letter string
3. `void displayBitPattern(int arr[])` - this function displays the bit pattern of the bit vector array

## Notes:

1. Remember to terminate the end of the string with `\0`
2. Bit vector array is where each index of the array represents a boolean value: `0` or `1`
3. Remember one character is 8 bits, given a binary 01011001 is 89 in decimal, it represents the character 'Y'
...
char* decodeString(int arr[]) {
    static char decoded[10];
    int val, bitIndex = 0;
    
    for (int i = 0; i < 9; i++) {
        val = 0;
        for (int j = 0; j < 9; j++) {
            val = (val << 1) | arr[bitIndex++];
        }
        decoded[i] = (char)val;
    }
    decoded[9] = '\0';
...
![[../Media/Pasted image 20251210080132.png]]